...
function detectCategory(text: string): string | undefined {
  if (has(text, ...FORGOT)) return 'belongings';
  if (has(text, ...LOW_STOCK) || has(text, ...BUY_VERBS)) return 'shopping';
  if (has(text, ...MESSAGE)) return 'message';
  return 'errand';
}

/** 自然文 → Moment候補（確信度の高い順）。 */
export function interpret(rawText: string): MomentInterpretation {
  ...
  if (drafts.length === 0) {
    ...
    if (isForgot) {
      ambiguityNote = 'どこに置いてきた? 場所を選んでください';
    } else {
      ambiguityNote = 'いつ思い出したい? どれかを選んでください';
    }
